(() => {
  ...
  if (!container) return;
  const allDeck = Array.from(container.querySelectorAll('.card'));
  let deck = [...allDeck];
  ...
  function render() {
    stopAudio();
    allDeck.forEach(card => { card.hidden = true; card.classList.remove('is-flipped'); card.tabIndex = -1; });
    const activeCard = current();
    if (activeCard) { activeCard.hidden = false; activeCard.tabIndex = 0; }
    visited.add(index);
  ...
  function buildCategoryFilters() {
    const knowledgeCards = allDeck.filter(card => card.dataset.knowledge === 'true');
    if (!knowledgeCards.length || mode !== 'trivia') return;
    const stage = document.querySelector('.deck-stage');
    const progress = stage?.querySelector('.progress-row');
    if (!stage || !progress) return;
    const filters = [
      ['all', 'All', allDeck.length],
      ['knowledge', 'Knowledge', knowledgeCards.length],
      ['History', 'History', knowledgeCards.filter(c => c.dataset.category === 'History').length],
      ['Natural Geography', 'Geography', knowledgeCards.filter(c => c.dataset.category === 'Natural Geography').length],
      ['Science', 'Science', knowledgeCards.filter(c => c.dataset.category === 'Science').length]
    ];
    const strip = document.createElement('div');
    strip.className = 'deck-filter-strip';
    strip.setAttribute('aria-label','Trivia category filter');
    strip.innerHTML = filters.map(([value,label,count],i) => `<button type="button" class="deck-filter ${i===0?'active':''}" data-filter="${escapeHtml(value)}">${escapeHtml(label)} <b>${count}</b></button>`).join('');
    stage.insertBefore(strip, progress);
    strip.addEventListener('click', e => {
      const btn = e.target.closest('[data-filter]');
      if (!btn) return;
      const value = btn.dataset.filter;
      if (value === 'all') deck = [...allDeck];
      else if (value === 'knowledge') deck = allDeck.filter(card => card.dataset.knowledge === 'true');
      else deck = allDeck.filter(card => card.dataset.category === value);
      index = 0;
      visited = new Set([0]);
      strip.querySelectorAll('.deck-filter').forEach(b => b.classList.toggle('active', b === btn));
      render();
      toast(`${btn.childNodes[0].textContent.trim()} deck • ${deck.length} cards`);
    });
  }

  normalizeCards(); buildCategoryFilters(); renderPlayers(); render();
