...
const CATEGORY_LABELS = {
  random: 'All regions', europe: 'Europe', 'north-america': 'North America',
  'south-america': 'South America', africa: 'Africa', oceania: 'Oceania',
  asia: 'Asia', antarctica: 'Antarctic claims'
};
...
const playerData = () => { try { return JSON.parse(localStorage.getItem('partyplay.players') || '[]').map(p => typeof p === 'string' ? {name:p,score:0} : p); } catch { return []; } };
const savePlayers = p => localStorage.setItem('partyplay.players', JSON.stringify(p));
...
function allFlags(){ return Object.entries(flagsByContinent).flatMap(([region,items]) => items.map(item=>({...item,region}))); }
function deckFor(category){ return category==='random' ? allFlags() : (flagsByContinent[category]||[]).map(item=>({...item,region:category})); }
function toast(msg){ const t=el('toast'); t.textContent=msg; t.classList.add('show'); clearTimeout(toast.timer); toast.timer=setTimeout(()=>t.classList.remove('show'),1400); }
function renderPlayers(){ const p=playerData(); const box=el('playersList'); if(!p.length){box.innerHTML='<div class="empty">No players set. Add names above if you want PartyPlay to keep score.</div>';el('turnName').textContent='Casual play';return;} turnIndex=((turnIndex%p.length)+p.length)%p.length; box.innerHTML=p.map((x,i)=>`<div class="player ${i===turnIndex?'active':''}"><span>${escapeHtml(x.name)}</span><strong>${Number(x.score)||0}</strong></div>`).join(''); el('turnName').textContent=p[turnIndex].name; }
function rotateTurn(delta=1){const p=playerData();if(!p.length)return;turnIndex=(turnIndex+delta+p.length)%p.length;localStorage.setItem('partyplay.turnIndex',String(turnIndex));renderPlayers();}
function renderRegions(){ const box=el('regionButtons'); const categories=['random',...Object.keys(flagsByContinent)]; box.innerHTML=categories.map(c=>`<button class="region-btn ${c===activeCategory?'active':''}" data-category="${c}">${CATEGORY_LABELS[c]||c}</button>`).join(''); box.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>setCategory(b.dataset.category))); }
function setCategory(category){ activeCategory=category; flagDeck=shuffle(deckFor(category)); flagIndex=0; renderRegions(); renderFlag(); }
function renderFlag(){ if(!flagDeck.length){flagDeck=shuffle(deckFor(activeCategory));} const item=flagDeck[flagIndex]; const card=el('flagCard'); card.classList.remove('revealed'); const img=el('flagImage'); const placeholder=el('flagPlaceholder'); img.hidden=false; placeholder.hidden=true; img.src=item.flagURL; img.alt=`Flag to identify — ${CATEGORY_LABELS[item.region]||item.region}`; img.onerror=()=>{img.hidden=true;placeholder.hidden=false;}; el('countryName').textContent=item.country; el('regionName').textContent=CATEGORY_LABELS[item.region]||item.region; el('flagCounter').textContent=`${flagIndex+1} / ${flagDeck.length}`; el('progressFill').style.width=`${((flagIndex+1)/flagDeck.length)*100}%`; el('regionStat').textContent=CATEGORY_LABELS[activeCategory]||activeCategory; el('countStat').textContent=flagDeck.length; el('revealBtn').textContent='Reveal answer'; }
function reveal(){ const c=el('flagCard'); c.classList.toggle('revealed'); el('revealBtn').textContent=c.classList.contains('revealed')?'Hide answer':'Reveal answer'; }
function next(rotate=true){ flagIndex=(flagIndex+1)%flagDeck.length; if(rotate)rotateTurn(1); renderFlag(); }
function score(){ const p=playerData(); if(!p.length){toast('Add players to keep score');return;} p[turnIndex].score=(Number(p[turnIndex].score)||0)+1; savePlayers(p); toast(`+1 ${p[turnIndex].name}`); next(true); }
el('flagCard').addEventListener('click', reveal); el('revealBtn').addEventListener('click',reveal); el('nextBtn').addEventListener('click',()=>next(true)); el('skipBtn').addEventListener('click',()=>next(true)); el('correctBtn').addEventListener('click',score); el('shuffleBtn').addEventListener('click',()=>{flagDeck=shuffle(flagDeck);flagIndex=0;renderFlag();toast('Flags shuffled');}); el('resetScoresBtn').addEventListener('click',()=>{savePlayers(playerData().map(p=>({...p,score:0})));renderPlayers();toast('Scores reset');}); window.addEventListener('partyplay:players-changed',renderPlayers);
document.addEventListener('keydown',e=>{if(e.key==='ArrowRight')next(true);else if(e.code==='Space'||e.key==='Enter'){e.preventDefault();reveal();}});
renderPlayers(); renderRegions(); setCategory('random');
